...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id FROM user.member WHERE account = ?");
    ...
    $stmt = $conn->prepare("SELECT 餐廳名 FROM 名單 WHERE 編號 = ?");
    ...
    if ($action === 'add') {
        $stmt = $conn->prepare("INSERT INTO favorites (user_id, restaurant_id, restaurant_name) VALUES (?, ?, ?)");
        $stmt->bind_param("iis", $user_id, $restaurant_id, $restaurant_name);
        $stmt->execute();
        $stmt->close();
    } elseif ($action === 'remove') {
        $stmt = $conn->prepare("DELETE FROM favorites WHERE user_id = ? AND restaurant_id = ?");
